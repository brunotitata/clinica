...
public class AgendaService {
    ...
    public Agenda registrarAgenda(Agenda obj) {
        ...
        agenda.setValorAgendamento(
                obj.getValorAgendamento().setScale(2, BigDecimal.ROUND_UP));
        agenda.setObservacoes(obj.getObservacoes());
    ...
    public List<Agenda> buscarAgendamentoPorNomePaciente(String nomePaciente) {
        return agendaRepository
                .findByNomePacienteContaining(nomePaciente.toUpperCase());
    }

    public List<Agenda> buscarAgendamentoPorProcedimento(String procedimento) {
        return agendaRepository
                .findByProcedimento(Procedimento.capturarValor(procedimento));
    }

    public void deletarAgendamento(Long id) {

        Agenda agenda = agendaRepository.findById(id)
                .orElseThrow(() -> new AgendamentoNãoEncontradoException(
                        "Não foi possivel encontrar agendamento com o id: "
                                + id));
    ...
    public Agenda editarAgendamento(Agenda obj) {
        ...
        Agenda agenda = agendaRepository.findById(obj.getId())
